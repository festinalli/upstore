						...
						<div class="col-md-6 pr-1">
							...
							<p><b>
								...
								@elseif($manutencao->status == 'PROPOSTA_ENVIADA' && $manutencao->orderServico)
									@if($manutencao->orderServico->metodo_pagamento == "LOJA")
									Cliente solicitou pagamento pela loja, confirme o recebimento para dar continuidade à manutenção
									<form action="{{ route('admin.manutencao.order.pagar', $manutencao->orderServico->id) }}" method="POST">
										@csrf
										<button type="submit" class="btn btn-primary">Clique para confirmar recebimento</button>
									</form>
									@else
