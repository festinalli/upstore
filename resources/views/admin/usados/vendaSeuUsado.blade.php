...
@section('main')
	<div class="main-panel">
      <!-- Navbar -->
      @include('admin.navbar', ['title' => 'Venda seu usado'])
      <!-- End Navbar -->
              ...
              <div class="card-header">
                ...
                <div class="row"> 
                  ...
                  <div class="col-md-2">
                    <div class="row">
                      <div class="col-lg-12 col-md-12 col-sm-12">
                        <select id="selecionaStatus" class="selectpicker" data-style="btn btn-default btn-round" multiple title="Selecione o status" data-size="7">
                ...
                <table id="datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                  ...
                  <tbody>
                    @foreach($servicos as $servico)
                      <tr style="cursor: pointer;" onclick="location.href='{{ route('admin.usado',['servico_id' => $servico->id]) }}'">
                        <td>V-{{ $servico->id }}</td>
                        <td>{{ $servico->cliente ? $servico->cliente->nome : '-'}}</td>
                        <td>{{ $servico->tecnico ? $servico->tecnico->nome : 'Sem responsável'}} </td>
                        <td>{{ date('d/m/Y',strtotime($servico->created_at)) }}</td>
                        <td>{{ $servico->data_entrega ? date('d/m/Y',strtotime($servico->data_entrega)) : '-'}}</td>
                        <td>R$ {{ number_format($servico->valor/100,2,',','.') }}</td>
